...
1. **Caller-asserted `asOf`, unverified.** §2.4's `evaluateE85DesignationLegalTextTemporalCoincidence` receives an explicit `asOf: string` argument and *assumes, but cannot verify*, that both supplied applicability results (§2.2, §2.3) were themselves computed against that same date. It validates only that `asOf` is a non-empty string and echoes it back as `.asOf`; it never compares it against an `asOfDate` embedded in either input result. Roughly half the variants on each side carry no embedded date field at all (e.g. `DESIGNATION_APPLICABILITY_NOT_EVALUABLE`, `LEGAL_TEXT_NO_START_EVIDENCE`, `LEGAL_TEXT_PARTIAL_TERMINATION_INDETERMINATE`), so such a comparison would be unavailable for those variants; for the rest, adding one would reverse the documented contract and the existing test that pins acceptance of a differing `asOf` (`designation-legal-text-temporal-coincidence.test.ts`). This remains a caller responsibility — documented, not enforced.
2. **No cross-record verification in the pairing wrapper.** §2.5's `pairE85DesignationLegalTextCorrespondenceAndCoincidence` cannot verify that the supplied `correspondence` and `coincidence` results refer to the same underlying designation record, legal-text/linkage entry, or as-of date — that is solely the calling code's responsibility. Its inputs do not carry the fields needed: the correspondence result has no date field in any variant, and the coincidence result carries no designation or legal-text identity. Beyond the shared `kind` field (and the variant-specific `limitation` field where present), `designationResultKind` is the only field whose values are shared between these results; it identifies a result *category*, not a record — two different designations with the same kind are indistinguishable by it.
3. **No cross-record verification in the orchestrator.** §2.5's `evaluateE85DesignationLegalTextConsistentPair` guarantees consistency only among its own five/three invocations over one shared input bundle — a mechanical, construction-guaranteed fact about that single call: the same `resolvedRequest` object reference is passed to both applicability calls, and the AS_OF date passed to the coincidence call is read from that same object. A further equality check among those dates would only repeat that guarantee. It cannot and does not verify that the caller originally assembled the designation validity, legal-text identity, linkage identity, version validity, or resolved temporal request from the same real-world records.
4. **No machine clock in the temporal chain.** None of §2.1–§2.5, nor the builders `buildE85DesignationValidity`/`buildE85VersionValidity` or `resolveE85TemporalRequest`, reads a machine clock; every date used is either caller-supplied (`asOf`) or already embedded in an already-validated/already-resolved input object. `ABSENT`/`CURRENT` temporal-request modes are never defaulted to "today" — they deterministically short-circuit to a dedicated not-evaluable/omitted outcome instead. **This claim is scoped to the temporal chain; E85 as a whole is not clock-free.** Older evaluator layers stamp wall-clock audit timestamps via `new Date()` — `resolvedAt` (`evaluator.ts`) and `checkedAt`/`flaggedAt` (`density-evaluation.ts`, `dimensional-evaluation.ts`, `overlay-evaluation.ts`, `requirement-evaluation.ts`, `rule-applicability.ts`). These record when an evaluation ran; they are not as-of dates and play no part in temporal applicability.

**What identity correspondence does and does not establish.** §2.1's `evaluateE85DesignationLegalTextIdentityCorrespondence` compares the supplied jurisdiction, source, source-version, and zone fields exactly and reports any mismatch as a result kind (`JURISDICTION_MISMATCH`, `SOURCE_MISMATCH`, `VERSION_MISMATCH`, `ZONE_MISMATCH`), not as a thrown error. This is equality over caller-supplied strings; it does not establish that those values came from the same real-world records. Separately, `E85VersionValidity` has no identity field linking it to the selected legal-text source/version; the `authoritySourceId`/`authoritySourceVersionId` fields on its start/end authorities identify the authority instrument and must not be read as the legal text's own identity. No current contract lets any function in the chain bind a version validity to a legal-text identity.

**Decided: no Phase 9 integration of the chain under current contracts.** Confirmed against source at `1a74cc96`: no Phase 9 module (`decision-orchestrator.ts`, the materiality modules) or `index.ts` imports any §2.1–§2.5 function, so the chain does not currently feed Phase 9. Under the chain's current contracts it must not affect Phase 9 `decisionStatus` or materiality, and its results are not to be attached to the decision package as diagnostics either: the assumption that callers vouch for the alignment of designation, legal text, linkage, validity, and as-of date is not explicit in the contract, and no result is marked as diagnostic. The existing lineage-based temporal gate (`computeE85TemporalMaterialityAddition` in `decision-orchestrator.ts`, via `decision-temporal-materiality-adapter.ts`) is unchanged by this decision. It emits only `MATERIAL` `GAP`/`MANUAL_REVIEW` records when a `temporalRequest` is explicitly supplied, so it can block a package but never clears or relaxes one. Integration is to be revisited only once the contract can represent and establish designation, legal-text, linkage, and as-of consistency. This decision authorizes no rule-pack application, legal conclusion, public export, or API exposure. It is separate from the open token design below and from the public-exposure prerequisites in §6.

**Unresolved.** Whether stronger runtime guarantees are required — and, if so, any shared identity/date token carried through the chain's result types — is an open contract decision. Nothing in this section approves such a token, any change to the frozen behavior of §2.1–§2.5 or their tests, or any export of the chain.
